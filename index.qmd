...
// Load color_ranks.csv
raw = await d3.csv("data/color_ranks.csv");

// Parse hex -> RGB/HSL
colors = raw.map(d => {
  const hex = d.hex?.trim();
  const name = d.color ?? d.name ?? "unknown";
...
// Add hue families
allColors = colors.map(d => {
  const h = d.h, s = d.s, l = d.l;
  let family;
  if (s < 10 || l > 95) family = "Grays/Whites";
  else if (h < 15 || h >= 345) family = "Reds";
  else if (h < 45) family = "Oranges";
  else if (h < 70) family = "Yellows";
  else if (h < 170) family = "Greens";
  else if (h < 200) family = "Cyans";
  else if (h < 255) family = "Blues";
  else if (h < 290) family = "Purples";
  else if (h < 330) family = "Pinks";
  else family = "Reds";
  return {
    color: d.color,
    hex: d.hex,
...
> Something that could be done is adding a 'max saturation' slider, as at the moment we only filter by minimul saturation. a maximum saturation slider could sllow us to focus on a specific saturation band, helping to reveal patterns that can be hidden when very dull/very bright colors might take over the graph. We could also use faceting or small multiples, maybe splitting the plot by hues. Another way to improve would be using a 2D density plot, which could keep the interactivity but reduce overplotting. 

Is your proposed graphic supported by vega-lite? Why or why not? (You can provide links to documentation/stackOverflow)

> Yes, most of these ideas are fully supported:
  Facets: https://vega.github.io/vega-lite/docs/facet.html
  Sliders: https://vega.github.io/vega-lite/docs/selection.html
While density plots is not directly supported by Vega-lite, this can be generated in R/JavaScript and can be fed in. 
...
viewof maxSat2 = Inputs.range([0, 100], {label: 'Max Saturation (%)', value: 100, step: 1})
